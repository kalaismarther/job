...
class EditJobBenefit extends StatefulWidget {
  const EditJobBenefit({super.key, required this.editData});

  final Map editData;

  @override
  State<EditJobBenefit> createState() => _EditJobBenefitState();
}

class _EditJobBenefitState extends State<EditJobBenefit> {
  List benefit = [];
  List selectedType = [];
  ...
  initilize() async {
    if (widget.editData['is_job_benefits'].runtimeType != List) {
      setState(() {
        selectedType = [];
      });
    } else {
      setState(() {
        selectedType = widget.editData['is_job_benefits'] ?? [];
      });
    }
