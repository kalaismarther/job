...
class EditEmployment extends StatefulWidget {
  const EditEmployment({super.key, required this.type, required this.data});

  final String type;
  final Map data;

  @override
  State<EditEmployment> createState() => _EditEmploymentState();
}
  ...
  initilize() {
    if (widget.type == "edit") {
      setState(() {
        ...
        company_name.text = widget.data['company_name'] ?? "";
        start_yr.text = (widget.data['start_year'] ?? "").toString();
        end_yr.text = (widget.data['end_year'] ?? "").toString();
        isCurrent = (widget.data['is_current'] ?? 0) == 1 ? true : false;
        hideSalary = (widget.data['is_current'] ?? 0) == 1 ? true : false;
        salary.text = (widget.data['salary_annual'] ?? "").toString();
        currency.text = widget.data['is_salary_currency'] ?? "";
        s_noticePeriod = (widget.data['notice_months'] ?? "").toString();
        s_currencyId = (widget.data['salary_currency_id'] ?? "").toString();
        if (widget.data['start_month'] != null) {
          start_month.text = getMonthById(months, widget.data['start_month']);
          s_startMonth = (widget.data['start_month'] ?? "").toString();
        }
        if (widget.data['end_month'] != null) {
          end_month.text = getMonthById(months, widget.data['end_month']);
          s_endMonth = (widget.data['end_month'] ?? "").toString();
        }
        if (widget.data['notice_months'] != null) {
          notice_period.text =
              getMonthById(notice_period_list, widget.data['notice_months']);
        }
      });
    }
  }
                                ...
                                if (_formKey.currentState!.validate()) {
                                  ...
                                  if (startDate.isAfter(endDate)) {
                                    ...
                                    var data = {
                                      ...
                                      "employment_id": widget.type == "edit"
                                          ? widget.data['id']
                                          : ""
                                    };
                                    print(data);
